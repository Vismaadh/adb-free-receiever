    ...
    private void handle(Socket socket) {
        try (Socket s = socket) {
            ...
            if ("PUT".equals(method) && target.startsWith("/upload/")) {
                ...
                MessageDigest digest = verify ? newSha256() : null;

                long startOffset = 0L;
                String startHeader = getHeader(lines, "X-Start-Offset");
                if (startHeader != null && !startHeader.isEmpty()) {
                    try {
                        startOffset = Long.parseLong(startHeader);
                    } catch (NumberFormatException e) {
                        sendText(out, 400, "Invalid start offset");
                        return;
                    }
                }
                ...
                // The final file must never be used as a resume source.
                // Resumable data lives only in .part until the complete body
                // has arrived.
                ...
                if (startOffset == 0) {
                    ...
                    sendText(out, 409, "PART_OFFSET_MISMATCH|" + existingPart);
                    return;
                }

                // If SHA-256 verification is requested and we are resuming,
                // include the already-received bytes in the digest.
                if (verify && existingPart > 0) {
                    try (InputStream previous = new BufferedInputStream(new FileInputStream(tempFile))) {
                        byte[] previousBuffer = new byte[64 * 1024];
                        int n;
                        while ((n = previous.read(previousBuffer)) != -1) {
                            digest.update(previousBuffer, 0, n);
                        }
                    }
                }

                try (FileOutputStreamCompat fos = new FileOutputStreamCompat(
                        tempFile, startOffset > 0)) {
                ...
                // If the full file has now arrived, atomically-ish finalize it.
                // A zero-byte resumed request can also finalize a complete .part.
                if (tempFile.length() < startOffset + contentLength) {
                    throw new IOException("Incomplete upload");
                }

                String sha = verify ? hex(digest.digest()) : null;

                if (file.exists() && !file.delete()) {
                    throw new IOException("Could not replace existing destination");
                }
                if (!tempFile.renameTo(file)) {
                    throw new IOException("Could not finalize uploaded file");
                }

                if (verify) sendText(out, 200, "OK|SHA256|" + sha);
                else sendText(out, 200, "OK");
                return;
            }
