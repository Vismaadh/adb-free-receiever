    ...
    private void handle(Socket socket) {
        try (Socket s = socket) {
            ...
            if ("PUT".equals(method) && target.startsWith("/upload/")) {
                ...
                // Never expose a partially uploaded file as the real destination.
                // The .part file is truncated/restarted on every retry and is
                // renamed to the final name only after the complete body arrives.
                File tempFile = new File(file.getPath() + ".part");

                try (FileOutputStreamCompat fos = new FileOutputStreamCompat(tempFile)) {
                    ...
                    // Keep the .part file for diagnostic purposes, but never
                    // leave it visible as the completed destination.
                    throw e;
                }

                String sha = verify ? hex(digest.digest()) : null;

                // Only now replace the final destination. This means an
                // interrupted upload can never make /exists report a real
                // completed file during the PC's retry.
