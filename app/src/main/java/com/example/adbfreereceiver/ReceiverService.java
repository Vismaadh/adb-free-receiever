...
public class ReceiverService extends Service {
    ...
    private void handle(Socket socket) {
        try (Socket s = socket) {
            ...
            // Browse arbitrary readable external-storage entries for the PC picker.
            if ("GET".equals(method) && target.startsWith("/browse")) {
                String relative = queryParam(target, "path");
                if (relative == null) relative = "";
                File dir = safeExternalFile(relative);
            ...
            if ("GET".equals(method) && target.startsWith("/download-any/")) {
                ...
                if (q >= 0) encoded = encoded.substring(0, q);
                String relative = URLDecoder.decode(encoded, "UTF-8");
                File file = safeExternalFile(relative);
                if (file == null || !file.isFile()) {
                    sendText(out, 404, "Not Found");
                    return;
                }
    ...
    private String queryParam(String target, String name) {
        ...
        for (String part : query.split("&")) {
            ...
            try {
                ...
                if (name.equals(k)) return URLDecoder.decode(part.substring(eq + 1), "UTF-8");
            } catch (Exception ignored) {}
        }
        return null;
    }

    private File safeExternalFile(String relative) throws IOException {
        if (relative == null) relative = "";
        ...
        if (!file.getPath().equals(rootPath) && !file.getPath().startsWith(rootPath + File.separator)) return null;
        return file;
    }

    private void sendBrowse(OutputStream out, File dir) throws IOException {
        ...
        if (children != null) {
            ...
            for (File f : children) {
                try {
                    ...
                    body.append(type).append("|").append(f.length()).append("|")
                            .append(URLEncoder.encode(rel, "UTF-8")).append("|")
                            .append(URLEncoder.encode(f.getName(), "UTF-8")).append("\n");
                } catch (Exception ignored) {}
            }
        }
